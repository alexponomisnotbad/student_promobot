...
uint8_t _twonode_queue_size_prm = 10;
 uint8_t _twonode_member_queue_size = 0;
...
public:
  ...
  : Node("second_node")
  {
    this->declare_parameter("_twonode_queue_parameter", _twonode_queue_size_prm);
    
    _twonode_member_queue_size = this->get_parameter("_twonode_queue_parameter").as_int();
    _psubscription = this->create_subscription<std_msgs::msg::String>(
      c_topic, _twonode_member_queue_size, std::bind(&TwoNode::topicCallback, this, std::placeholders::_1));
    _ppublisher = this->create_publisher<std_msgs::msg::Int32>(c_subtopic, _twonode_member_queue_size);
  }
